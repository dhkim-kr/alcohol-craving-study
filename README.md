# Wearable Multimodal Physiomarker Identification for Graded Alcohol Craving

Code release accompanying the paper (TODO: update with the final title/venue/DOI once available):

> D.H. Kim, M.S. Lee, D.-H. Lee, J.-Y. Lee, J.E. Han, M. Kwon, J.Y. Hwang, J.H. Kim, S.K. Lee, Y.-S. Choi.
> *Wearable Multimodal Physiomarker Identification for Graded Alcohol Craving During VR-Based Cue Elicitation.*
...
2. `data/alcohol_Qscores.csv`, `data/alcohol_scenarios.csv`, and `data/labels/alcohol.json` (craving labels) drive the label/scenario side and are already provided (the `.json` is not tracked by git by default — see `data/README.md`).
...
Not applicable — every classifier (KNN/SVM/LR/MLP/XGB/RF) is trained from scratch inside `notebooks/05_ml_classification_lmm_shap.ipynb` in a few minutes on the extracted feature table; there are no separate checkpoint files to download.
...
- **Raw physiological data**: not publicly available (IRB-approved clinical data collection; see `data/README.md`). TODO: add a data-access-request process if/when the authors decide to support one.
- **Paper**: TODO — link/DOI once published.

## License

TODO — not yet decided. Do not treat this repository as licensed for reuse until this section is filled in.

## Citation

TODO — add a `CITATION.cff` / BibTeX entry once the paper has a stable venue/DOI.
